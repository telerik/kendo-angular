# Using the Conversational Ui with Google Dialogflow V2

This sample project demonstrates how to use the [Kendo UI for Angular Conversational UI](https://www.telerik.com/kendo-angular-ui/components/conversationalui/) component with [Google Dialogflow V2](https://cloud.google.com/dialogflow/docs/reference/rest/v2-overview). The project uses an Angular 9 client side application and an Express server, which is configured to use a test service account.
...
1. Clone the sample app repository locally by using your favorite Git client by running `git clone https://github.com/telerik/kendo-angular/.git`.
1. Use the agent.zip file (located in the project root folder) to create a new Agent by [importing the archive in the Google Dialogflow console](https://cloud.google.com/dialogflow/docs/agents-settings#export).
1. [Get a service account key](https://dialogflow.com/docs/reference/v2-auth-setup?authuser=2#getting_the_service_account_key) by exporting its key JSON file and save it.
1. Rename the key JSON file to `credentials.json` and add it to the project `backend/server` folder.
1. Deploy the Fulfillment code using the [Inline Editor](https://cloud.google.com/dialogflow/docs/fulfillment-inline-editor)
    ```
    ...
    ```
1. Enter the project server directory `insurance/backend/server/`.
1. Run `npm install` and `npm start` to start the server.
1. Enter the project client directory `insurance/example/angular-client/`
1. Run `npm install` and `ng serve` to start the client app.
